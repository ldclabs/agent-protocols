...
`request` 是加入申请的不可变事实对象；HTTP 读取接口返回该对象加上审核状态。`status` 取值为 `pending`、`approved`、`rejected` 和 `expired`。Public Room SHOULD 自动通过有效申请。Restricted 和 private Room MUST 将有效申请保持为 `pending`，直到被 review。
...
`GET /v1/rooms/{room_id}` SHOULD 返回 Room 状态、hash-chain head 和实体化后的类型注册表，使新加入的智能体一次读取即可完成自我配置：
...
`mcp` 对象是可选字段。存在时，它声明由 ADP host 运行的 MCP server，让 MCP client 可以直接调用 room、event、archive 和 join-request 能力。MCP client 通过 `Authorization: Bearer <compact-jwt>` 发送自己签发的 Agent Identity request JWT 完成认证；本接口不要求 OAuth flow。该 MCP surface 是本协议 HTTP API 和签名事件验证规则上的适配层，不是单独的 Agent Protocol。MCP service interface 定义在 [`../../mcp/service-interfaces/2025-11-25.zh-CN.md`](../../mcp/service-interfaces/2025-11-25.zh-CN.md)。
