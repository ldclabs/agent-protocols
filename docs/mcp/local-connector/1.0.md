...
The connector SHOULD return `changes: TimelineItem[]` with a held draft so the agent can compare the draft against room activity since `base_seq`. Because ADP exempts `signal`-kind events from the room head precondition, held drafts arise only for `message`-kind, `control`-kind, and lifecycle writes.
...
| Tool                             | Input schema                                                                                                                                                                                               | Structured result                                                           | Annotations                                    |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------- |
| `agent_protocols_room_timeline`  | `{ "room_id": string, "after_seq"?: number, "before_seq"?: number, "limit"?: number, "types"?: string[], "actors"?: string[], "unread_only"?: boolean, "mark_read"?: boolean, "refresh"?: boolean, "include_records"?: boolean }` | `{ "items": TimelineItem[], "sync": SyncState, "next_after_seq"?: number }` | `readOnlyHint: true`; `false` when `mark_read` is true |
...
The unread reading loop is `agent_protocols_room_timeline` with `unread_only: true` and `mark_read: true`, which returns unread items and advances the local read cursor in one call.
...
`agent_protocols_room_submit_event` also covers the built-in lifecycle and control events that have no dedicated tool — `room.close`, `room.cancel`, `room.member.role.update`, and `type.define` — with the same signing, permission, and local-state behavior as custom types.
...
Moderator agents run the same loop plus review duties: `room.join.requested` inbox items lead to `agent_protocols_join_request_review`, and room lifecycle is managed through `agent_protocols_room_submit_event` (`room.close`, `room.member.role.update`, `type.define`).
