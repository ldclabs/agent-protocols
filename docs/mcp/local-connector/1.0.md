...
- Agent Delegation Protocol defines principal resolution, controller authority, grants, revocations, and historical verification. The delegation tools below adapt those operations.
...
## 4. Connector Model and Operations
...
- Sign `profile.update`, `room.create`, `room.join`, and room event envelopes using Agent Identity canonicalization, SHA3-256 hashes, Ed25519 signatures, and monotonically increasing nonces, and sign `delegation.grant` and `delegation.revoke` envelopes only when the active Agent ID matches a current Controller record with sufficient `delegation` authority.
...
- Resolve principal documents per Agent Delegation Section 3 before relying on any delegation credential, and treat a document served away from its own `id` as carrying no controller binding.
...
Delegation reads are anchored on the principal, never on a service URL the caller was handed. `agent_protocols_principal_resolve` runs Agent Delegation Section 3 resolution — following redirects, discarding a document served away from its own `id`, and re-reading the canonical URL — and reports `alias: true` only when the requested URL differs from `canonical_id` and is listed in the resolved document's `aliases`. `agent_protocols_delegation_check` resolves `principal_id` the same way and then queries the `delegation_query_url` that the resolved document names; `subject` defaults to the active Agent ID, so the ordinary question — may this agent act for that principal? — is a single call.

`agent_protocols_delegation_check` sends only public existence queries, which carry both `subject` and `principal_id`. `agent_protocols_delegations_list` enumerates the active Agent ID's own delegations, which Agent Delegation Section 9.6 requires the service to authorize: the connector sends an Agent Identity request JWT for the delegation service origin, and MUST NOT enumerate any subject other than the active Agent ID.

`agent_protocols_delegation_check` returns existence-query candidates, not a blanket authorization verdict. Before relying on a candidate, the caller MUST apply Agent Delegation Section 8, including its configured relying-application audience, Controller policy, signed history, validity, constraints, and status. Neither an `active` label nor the controller’s source origin supplies these checks. Historical signatures use `retired_controllers` and acceptance evidence; a retired key cannot authorize a new submission.

`agent_protocols_delegation_grant` and `agent_protocols_delegation_revoke` sign with the active Agent ID. Before signing, the connector MUST resolve `principal_id`, substitute the canonical ID, and locate a current Controller whose `id` matches that Agent ID and whose `valid_from` has passed. Missing `delegation` means refusal. For a grant, the connector MUST require non-empty `audiences` and `scopes` and enforce the Controller’s delegation ceiling. For replacement or revocation, it MUST read the existing credential and enforce the immutable `owner_controller` rule: restricted keys manage only their own credentials; `delegation: "*"` may manage this principal’s other credentials.

The connector MUST validate the supplied `delegation_service` against the authoritative service discovered through the principal and operator host policy before signing or submitting. If required policy or ownership evidence is unavailable, it MUST refuse the write. The service still MUST perform its own acceptance checks. Both tools build the envelope internally and never accept `actor`, `nonce`, `created_at`, `hash`, or `signature` from the agent. These tools do not register controllers or modify their authority.
...
- Agent Identity request JWTs signed by a local connector prove control of an Agent ID to Agent Protocol services. They do not replace Agent Identity signatures required by Agent Profile, Agent Delegation, or Agent Discourse write operations.
...
This checklist summarizes the requirements above for the tool subset an implementation exposes:

- Keep keys local, build signed envelopes internally, and enforce operator host policy (Section 4.1).
- Match the documented input and result contracts, including delegation authority and audience checks (Section 4.3).
- Verify room history before updating local state; preserve presented-head semantics, held drafts, and explicit retry decisions (Sections 4.2–4.5).
- Return structured results and preserve protocol error codes (Section 5).
- Apply the transport authentication and untrusted-content boundaries in Sections 3 and 6.
